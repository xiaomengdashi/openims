...
int main(int argc, char** argv) {
  ...
  ims::sip::B2buaRelay relay(sip, ims::sip::B2buaRelayConfig{.realm = cfg.realm, .next_hop_uri = cfg.routing.icscf_to_scscf_uri});

  sip.set_on_message([&](const ims::sip::SipMessage& msg) { relay.on_message(msg); });
  if (!sip.start_udp(cfg.icscf.bind_ip, cfg.icscf.port)) return 2;
  ims::core::log()->info("icscfd started {}:{}", cfg.icscf.bind_ip, cfg.icscf.port);
